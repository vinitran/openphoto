...
function toPlan(result: AiResult): EditPlan {
  ...
  const maskCommands: EditCommand[] = result.regions.map(region => ({
    ...
    parameters:{id:region.id,name:region.name,type:'semantic',feather:region.feather,geometry:{polygon:region.polygon,semanticLabel:region.semanticLabel} as JsonValue},
  }));
  const adjustmentCommands: EditCommand[] = result.adjustments.map(item => {
    ...
    const value=Math.max(definition.min,Math.min(definition.max,item.value));
    return{id:createId('cmd'),version:1,tool:item.tool,target:item.targetId?{type:'mask',id:item.targetId}:{type:'document'},parameters:{value},origin:'ai'};
  });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},signal:AbortSignal.timeout(45_000),body:JSON.stringify({
      ...
      input:[{role:'user',content:[{type:'input_text',text:body.prompt},{type:'input_image',image_url:body.image,detail:'high'}]}],
